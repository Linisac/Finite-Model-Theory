...
%%
\end{enumerate}
(note that the sentence is in $\dlog{\omega_1}$) and, using Lemma 2.2.4(b), yet another equivalent statement, in parallel to Corollary 2.3.4(ii):
\begin{enumerate}[(1)]
\setcounter{enumii}{5}
%%
...
\end{enumerate}
%
\item \header{Note on Part (a) of 3.3.9 and 3.3.10} It implies that
\begin{center}
\begin{tabular}{ll}
...
iff & for every $m \in \nat$, $\strct{A} \equv^s_m \strct{B}$ \cr
iff & for every $m \in \nat$, $\strct{B} \satis \ityp[s]{m}{\strct{A}}$ \cr
iff & $\strct{B} \satis (\bland\limits_{m \in \nat} \ityp[s]{m}{\strct{A}})$ \cr
\end{tabular}
\end{center}
for 3.3.10.
%
