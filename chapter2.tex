...
%
\item \header{Note on Players' Winning Strategy for Ehrenfeucht-Fra\"iss\'e Game} By definition, every round of the game $\game{m}(\strct{A}, \vect{a}, \strct{B}, \vect{b})$ concludes with exactly one winner, either the spoiler or the duplicator.
...
\newpar
The proof in text implies that \emph{the following two statements are equivalent:
...
To see (iv) implies (v), simply take $\varphi \defas \hint{m}{\strct{A}, \vect{a}}$. The direction from (v) to (iv) is similar to the part of for the direction from (i) to (iii) in text. In particular, for the inductive case $m > 0$, if $\varphi = \neg\psi$, then we proceed to $\psi$. If $\varphi = \psi \lor \chi$, then we proceed to one of the disjuncts $\psi$ and $\chi$ that distinguishes $(\strct{A}, \vect{a})$ from $(\strct{B}, \vect{b})$ in terms of satisfaction. Thus, we can focus on $\varphi = \exists y \psi$. We distinguish two cases:
\begin{itemize}
%%%
\item If $\strct{A} \satis \varphi[\vect{a}]$ and $\strct{B} \not\satis \varphi[\vect{b}]$, then there is an $a \in A$ such that for all $b \in B$, it holds that $\strct{A} \satis \psi[\vect{a}a]$ and $\strct{B} \not\satis \psi[\vect{b}b]$, which by induction hypothesis implies that the spoiler wins the game $\game{m - 1}(\strct{A}, \vect{a}a, \strct{B}, \vect{b}b)$.
%%%
\item If $\strct{A} \not\satis \varphi[\vect{a}]$ and $\strct{B} \satis \varphi[\vect{b}]$, then there is a $b \in B$ such that for all $a \in A$, it holds that $\strct{A} \not\satis \psi[\vect{a}a]$ and $\strct{B} \satis \psi[\vect{b}b]$, which by induction hypothesis implies that the spoiler wins the game $\game{m - 1}(\strct{A}, \vect{a}a, \strct{B}, \vect{b}b)$.
%%%
\end{itemize}
Collectively, we have that \emph{there is a an $a \in A$ such that for all $b \in B$ the spoiler wins $\game{m - 1}(\strct{A}, \vect{a}a, \strct{B}$ or there is a $b \in B$ such that for all $a \in A$  the spoiler wins $\game{m - 1}(\strct{A}, \vect{a}a, \strct{B}$}. Hence, by 2.2.4(b) (also see the discussion in \textbf{Note on Lemma 2.2.4}), we have that the spoiler wins $\game{m}(\strct{A}, \vect{a}, \strct{B}, \vect{b})$.
\newpar
The above discussion now makes it clear that a formula $\varphi$ that distinguishes $(\strct{A}, \vect{a})$ from $(\strct{B}, \vect{b})$ can be turned into a winning strategy of the game $\game{m}(\strct{A}, \vect{a}, \strct{B}, \vect{b})$ for spoiler which consists of always choosing an appropriate element for the existential quantifier $\exists$ from the corresponding structure. More specifically, for $\varphi = \exists y \psi$, if $\strct{A} \satis \varphi[\vect{a}]$ and $\strct{B} \not\satis \varphi[\vect{b}]$, then choose a suitable $a \in A$; if $\strct{A} \not\satis \varphi[\vect{a}]$ and $\strct{B} \satis \varphi[\vect{b}]$, then choose a suitable $b \in B$.
%
...
%
\item \header{Note on the Isomorphism Type $\itset{m}(\strct{A}, \vect{a})$ as a Set} One can show by induction on $m$ that for all $\strct{A}, \vect{a} \in A$ and all $\strct{B}, \vect{b} \in B$ that
\begin{center}
