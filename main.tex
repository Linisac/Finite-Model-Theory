...
%%%Ehrenfeucht-Fraisse game, pebble game
%%%isomorphism types
...
%%%logic operations
%%%finite notions
%%%random notions and theory
%%%nullary relations and predicates
%%%operations on formulas with structures
%%%global relations
...
\newcommand{\from}{\leftarrow} %the inverse of \to
%% >
...
\newcommand{\sett}[2]{{\{#1 \mid \mathtext{#2}\}}} %set by text description. sett{#1}{#2} = {#1 | #2}, where #2 is a text
...
\newcommand{\cmps}{\mathop{\circ}} %binary operation for function composition
%% >

%%special set(s), theory <
\newcommand{\nat}{\mathbb{N}} %set of natural numbers
\newcommand{\zah}{\mathbb{Z}} %set of integers
\newcommand{\rat}{\mathbb{Q}} %set of rational numbers
...
\newcommand{\bdunion}{\mathop{\dot{\bigcup}}} %big disjoint union
...
%%elementary equivalence <
\newcommand{\equv}{\equiv} %equivalence (in a logic)
%% >

%%Ehrenfeucht-Fraisse game, pebble game <
\newcommand{\efgame}{\mathrm{G}} %prototype of Ehrenfeucht-Fraisse game
...
\newcommand{\off}{\ast} %off the board (in pebble games)
...
\newcommand{\itset}[1]{\mathrm{IT}^{#1}} %isomorphism type as a set
\newcommand{\ityp}[3][]{\ifthenelse{\equal{#1}{}}{{\psi^{#2}_{#3}}}{{^{#1}\psi^{#2}_{#3}}}} %(#2)-isomorphism type over #3 in other logics in #1 number of variables
%% >

%%graph, graph functions, graph relations <
\newcommand{\graph}{\mathrm{GRAPH}} %the set of finite graphs
\newcommand{\gr}{\mathit{G}} %notion of graphs
...
%%logics <
\newcommand{\folog}[1][]{\ifthenelse{\equal{#1}{}}{\mathrm{FO}}{\mathrm{FO}^{#1}}} %first-order logic optionally parameterized by number of variables
\newcommand{\solog}{\mathrm{SO}} %second-order logic
\newcommand{\msolog}{\mathrm{MSO}} %monadic second-order logic
\newcommand{\dqlog}[3][]{\ifthenelse{\equal{#1}{}}{\mathrm{L}_{#2#3}}{\mathrm{L}^{#1}_{#2#3}}} %logic parameterized by size of sets over which disjunction can be taken and depth of nested quantifiers and optionally number of variables
\newcommand{\dlog}[2][]{\dqlog[#1]{#2}{\omega}} %logic parameterized by size of sets over which disjunction can be taken and optionally number of variables
\newcommand{\inflog}[1][]{\dlog[#1]{\infty}} %infinitary logic optionally parameterized by #1 number of variables
...
\newcommand{\distinct}{\mathop{\mathrm{distinct}}} %distinct variables in formulas
...
\newcommand{\withassgn}[1]{[#1]} %with assignment #1; used in conjunction with \satis: \strct{A} \satis \varphi\withassgn{a} means the structure \strct{A} together with the assignment a satisfies \varphi
%% >
...
\newcommand{\randstrtheory}{\theory_\rand} %random structure theory
\newcommand{\infrandstr}{\strct{R}} %the infinite random structure
\newcommand{\randgrtheory}{\theory_{\rand, \gr}} %random graph theory
%% >
...
%%special classes of structures <
\newcommand{\ordclass}[1][]{\ifthenelse{\equal{#1}{}}{\mathrm{ORD}}{\mathcal{O}\of{#1}}} %class of finite ordered structures (in a vocabulary)
\newcommand{\even}{\mathrm{EVEN}} %class of finite structures of even cardinality (in a vocabulary)
\newcommand{\rigid}{\mathrm{RIG}} %class of finite rigid structures
%% >

%%extension axioms <
\newcommand{\extaxm}[1][]{\ifthenelse{\equal{#1}{}}{\chi}{\chi_{#1}}} %extension axiom with optional parameter #1 (set of formulas)
\newcommand{\conjextaxm}[2][]{\ifthenelse{\equal{#1}{}}{\epsilon_{#2}}{#1^{#2}}} %conjunction of extension axioms where variables indexed no more than #2; if #1 is present then this gives the conjunction of #1 with extension axioms where variables are indexed no more than #2 that are compatible with the sentence #1
%% >
...
\newcommand{\wrt}{\mid} %with respect to; the separator between two classes in probability notation
%% >
